...
export const MatchHistoryCard = ({ match, index }: MatchHistoryCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="card-surface rounded-xl p-5 hover:border-primary/30 transition-all duration-300"
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {format(new Date(match.timestamp), "MMM d, yyyy • h:mm a")}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {sortedPlayers.map((player) => (
          ...
          >
            ...
            <p className={cn(
              "text-xs font-medium",
              player.ratingChange >= 0 ? "text-emerald-light" : "text-destructive"
            )}>
      ...
      <div className="mt-3 flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>{match.rounds.length} rounds played</span>
      </div>
    </motion.div>
  );
};
